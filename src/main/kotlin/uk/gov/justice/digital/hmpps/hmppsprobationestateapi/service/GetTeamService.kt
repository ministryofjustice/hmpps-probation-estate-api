...
@Service
class GetTeamService(private val teamRepository: TeamRepository, private val probationDeliveryUnitRepository: ProbationDeliveryUnitRepository) {
  ...
  suspend fun findTeamDetailsByCode(teamCode: String): TeamDetails? = teamRepository
    .findById(teamCode)?.let {
      val probationDeliveryUnitOverview = probationDeliveryUnitRepository.findById(it.pduCode)!!.let { probationDeliveryUnit -> ProbationDeliveryUnitOverview(probationDeliveryUnit.code, probationDeliveryUnit.name) }
